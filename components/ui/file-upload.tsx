...
type Props = {
  // Server Action from useActionState(uploadImagesAction.bind(...))
  action?: (fd: FormData) => void | Promise<any>
  // Optional client callback if you want manual handling
  onChange?: (files: File[]) => void
  name?: string
  accept?: string
  multiple?: boolean
  // Render existing images grouped by sections
  sections?: Section[]
  // Optional remove handler for existing images
  onRemoveImage?: (img: ExistingImage) => void | Promise<any>
}
...
export function FileUpload({
  ...
  async function handleFileChange(newFiles: File[]) {
    ...
    // If a server action is provided, post multipart data
    if (action) {
        ...
        <div className="space-y-6">
          {sections.map((sec) => (
            <div key={sec.title} className="space-y-3">
              <h3 className="text-sm font-medium text-neutral-700 dark:text-neutral-200">{sec.title}</h3>
              {sec.images.length === 0 ? (
